...
def main(args):
   ...
   # === MLflow Setup - ESSENTIAL for experiment tracking ===
    ...
    tracking_uri = args.mlflow_uri or f"sqlite:///{project_root}/mlflow.db"
    ...
    with mlflow.start_run() as run:
        ...
        print("⚠️ Skipping Great Expectations validation...")
        ...
        mlflow.log_artifact(feature_txt)
        ...
        mlflow.log_artifact(preprocess_file)
        ...
        # ======================================================
        # COPY TRAINED MODEL TO src/serving/model
        # ======================================================

        print("\nCopying model for FastAPI serving...")

        client = MlflowClient()

        logged_model = client.search_logged_models(
            experiment_ids=[mlflow.get_experiment_by_name(args.experiment).experiment_id],
            max_results=1
        )[0]

        temp_dir = client.download_artifacts(
        ...
        shutil.copy2(
            os.path.join(project_root, "artifacts", "feature_columns.txt"),
            os.path.join(serving_dir, "feature_columns.txt")
        )

        shutil.copy2(
            os.path.join(project_root, "artifacts", "preprocessing.pkl"),
            os.path.join(serving_dir, "preprocessing.pkl")
        )
        
        print("✅ Serving model copied to:")
        print(serving_dir)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--input",
        type=str,
        default="data/processed/telco_customer_churn.csv",
        help="path to CSV"
    )
